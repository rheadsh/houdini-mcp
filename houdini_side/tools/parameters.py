...
from houdini_side.tools.common import as_text, set_node_parm, to_jsonable
    ...
    try:
        def work():
            ...
            if parm is None:
                ...
                return ok({"name": parm_name, "value": to_jsonable(pt.eval())})
            return ok({"name": parm_name, "value": to_jsonable(parm.eval())})
...
def _parm_get_all(node_path: str):
    try:
        def work():
            ...
            for p in node.parms():
                try:
                    params[p.name()] = to_jsonable(p.eval())
                except Exception:
                    params[p.name()] = None
...
def register(app):
    ...
    async def parm_set_expression(node_path: str, parm_name: str,
        ...
        return as_text(_parm_set_expression(node_path, parm_name, expr, language))
    ...
    async def parm_keyframe_set(node_path: str, parm_name: str,
        ...
        return as_text(_parm_keyframe_set(node_path, parm_name, frame, value))
    ...
    async def parm_keyframe_delete(node_path: str, parm_name: str, frame: float) -> list:
        ...
        return as_text(_parm_keyframe_delete(node_path, parm_name, frame))
    ...
    async def parm_keyframes_list(node_path: str, parm_name: str) -> list:
        ...
        return as_text(_parm_keyframes_list(node_path, parm_name))
    ...
    async def parm_link(src_node: str, src_parm: str,
        ...
        return as_text(_parm_link(src_node, src_parm, dst_node, dst_parm))
